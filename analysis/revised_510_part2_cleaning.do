...
capture drop date_covid_test
capture drop date_covid_hospital
...
order patient_id practice_id date_covid_test_pos date_covid_test_neg date_patient_index has_follow_up date_diabetes_diagnosis date_birth sex region date_deregistered date_death
sort patient_id

replace date_patient_index=min(date_covid_test_pos, date_covid_test_neg)
...
**// Exposure group
gen     group=.
replace group=1 if min(date_diabetes_diagnosis, date_t1dm_hospital_first, date_t2dm_hospital_first)<=date_patient_index & date_patient_index==date_covid_test_pos & ///
date_covid_test_pos<date_covid_test_neg
replace group=2 if min(date_diabetes_diagnosis, date_t1dm_hospital_first, date_t2dm_hospital_first)> date_patient_index & date_patient_index==date_covid_test_pos & ///
date_covid_test_pos<date_covid_test_neg
replace group=3 if min(date_diabetes_diagnosis, date_t1dm_hospital_first, date_t2dm_hospital_first)<=date_patient_index & date_patient_index==date_covid_test_neg & ///
date_covid_test_neg<date_covid_test_pos
label define grouplab 1 "Positive COVID test with diabetes" 2 "Positive COVID test without diabetes" 3 "Negative COVID test with diabetes"
label values group grouplab

**// Censoring
gen date_censor=min(date_deregistered, date_death, date_studyend)
**// Group 2 (censor at diabetes diagnosis)
replace date_censor=min(date_diabetes_diagnosis, date_t1dm_hospital_first, date_t2dm_hospital_first) ///
if min(date_diabetes_diagnosis, date_t1dm_hospital_first, date_t2dm_hospital_first)<date_censor & group==2
...
**// Ethnicity
gen cat_ethnic=6
capture replace cat_ethnic=ethnicity_gp  if ethnicity_gp >=1 & ethnicity_gp <=5
capture replace cat_ethnic=ethnicity_sus if ethnicity_sus>=1 & ethnicity_sus<=5 & cat_ethnic==6
...
capture drop ethnicity_gp 
capture drop ethnicity_sus
...
if _rc==0 {
	replace cat_smoking=1 if latest_smoking=="N" & ever_smoked!=1
	replace cat_smoking=2 if latest_smoking=="E" & ever_smoked==1
	replace cat_smoking=3 if latest_smoking=="S" & ever_smoked==1
...
if _rc==0 {
	gen     cat_bmi=1 if bmi>=10   & bmi<18.5
	replace cat_bmi=2 if bmi>=18.5 & bmi<25
	replace cat_bmi=3 if bmi>=25   & bmi<30
	replace cat_bmi=4 if bmi>=30   & bmi<100
	recode  cat_bmi .=5
...
label define cat_hba1clab 1 "< 42 mmol/mol" 2 "42 to 47" 3 ">= 48" 4 "Unknown"
